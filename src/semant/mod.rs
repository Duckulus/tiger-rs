...
impl<'a, F: Frame> Semant<'a, F> {
    ...
    fn trans_exp_rec(
        ...
        match exp.0.clone() {
            Exp::Var(var) => Ok(self.trans_var(*var, level)?),
            Exp::Nil => Ok((trans_nil(), Type::Nil)),
            Exp::Int(value) => Ok((trans_int(value), Type::Int)),
            Exp::String(s) => Ok((self.translator.trans_string(s), Type::String)),
            Exp::Call(fun_name, args) => {
                let fun_type = env_lookup(self.value_env, fun_name.clone())?;
                if let ValueEnvEntry::Fun(arg_types, return_type, fun_level, fun_label) = fun_type {
                    if args.len() != arg_types.len() {
                        let args_span = if args.is_empty() {
                            ...
                            SimpleSpan::from(
                                args.first().unwrap().1.start..args.last().unwrap().1.end,
                            )
                        };
            ...
            Exp::Op(op, lhs, rhs) => {
                let (left_exp, left_type) = self.trans_exp_rec(*lhs.clone(), level.clone())?;
                let (right_exp, right_type) = self.trans_exp_rec(*rhs.clone(), level)?;
                match op.0 {
                    ...
                    Oper::Eq | Oper::Neq => match left_type {
                        Type::Int | Type::String | Type::Record(_, _) | Type::Array(_, _) => {
                            if right_type == left_type
                                || (matches!(left_type, Type::Record(_, _))
                                    && right_type == Type::Nil)
                            {
                                Ok((
                                    trans_rel_op(op.0, left_type, left_exp, right_exp),
            ...
            Exp::If { cond, then, elsee } => {
                ...
                let (cond_exp, cond_type) = self.trans_exp_rec(*cond, level.clone())?;
                ...
                let (then_exp, then_type) = self.trans_exp_rec(*then, level.clone())?;
                if let Some(elsee) = elsee {
                    let else_span = elsee.1;
                    let (else_exp, else_type) = self.trans_exp_rec(*elsee, level)?;
            ...
            Exp::Array { typ, size, init } => {
                let typ_span = typ.1;
                let ty = actual_type(env_lookup(self.type_env, typ)?);
                if let Type::Array(ref elem_type, _) = ty {
                    let (size_exp, size_type) = self.trans_exp_rec(*size.clone(), level.clone())?;
                    ...
                    let (init_exp, init_type) = self.trans_exp_rec(*init.clone(), level)?;
    ...
    fn trans_var(&mut self, var: Var, level: Rc<Level<F>>) -> Result<TranslatedExp, TypeError> {
        match var {
            Var::Simple(symb) => {
                let symb_span = symb.1;
                let entry = env_lookup(self.value_env, symb)?;
            ...
            Var::Field(var, symb) => {
                let (var_exp, var_typ) = self.trans_var(*var, level)?;
            ...
            Var::Subscript(var, exp) => {
                let exp_span = exp.1;
                let (index_exp, index_type) = self.trans_exp_rec(*exp, level.clone())?;
        ...
        match dec {
            Dec::Function(funcs) => {
                ...
                for func in &funcs {
                    ...
                    for param in &func.params {
                        let param_type = actual_type(env_lookup(self.type_env, param.typ.clone())?);
                    ...
                    let return_type = func
                        .result
                        .clone()
                        .map(|t| env_lookup(self.type_env, t).map(actual_type))
                        .transpose()?
                        .unwrap_or(Type::Void);
                ...
                for (func, func_level) in funcs.into_iter().zip(func_levels) {
                    ...
                    for (param, access) in func.params.into_iter().zip(func_level.formals()) {
                        let param_type = actual_type(env_lookup(self.type_env, param.typ)?);
                        self.value_env
            ...
            Dec::Type(named_types) => {
                ...
                for ((name, span), type_decl) in &named_types {
                    let typ = self.trans_type_decl(type_decl.clone())?;
                    let placeholder_type = self.type_env.lookup(name).unwrap();
    ...
    fn trans_type_decl(&mut self, type_decl: TypeDecl) -> Result<TypeRef, TypeError> {
        match type_decl {
            TypeDecl::Name(name) => env_lookup(self.type_env, name),
            TypeDecl::Record(fields) => Ok(Rc::new(RefCell::new(Type::Record(
                fields
                    .into_iter()
                    .map(|field| {
                        let ty = env_lookup(self.type_env, field.typ)?;
                        Ok((field.name, ty))
                    })
            ...
            TypeDecl::Array(typ) => {
                let ty = env_lookup(self.type_env, typ)?;
